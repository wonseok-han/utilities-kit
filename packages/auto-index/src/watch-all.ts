 ...
 */
export function getAutoIndexConfig(): AutoIndexConfig | null {
  ...
  try {
    ...
    if (config) {
      ...
      if (
        ...
        mergedConfig.watchTargets = mergedConfig.watchTargets.map(
          (target: any) => ({
            ...target,
          })
        );
    ...
    console.error('package.json 읽기 오류:', errorMessage);
    return null;
  }
}
 ...
 */
function globToPaths(pattern: string, config: AutoIndexConfig): string[] {
  ...
  if (pattern.includes('**')) {
    ...
    if (parts.length === 2) {
      ...
      if (basePath !== undefined && targetFolder !== undefined) {
...
function findFoldersByName(
  ...
  results: string[] = [],
  config: AutoIndexConfig = DEFAULT_CONFIG
): string[] {
 ...
 */
export function extractPathsFromWatchTargets(
...
export function watchAllPaths(
  ...
  uniqueFolders.forEach((folder) => {
    ...
    const autoIndexPath = path.resolve(__dirname, '../bin/auto-index.js');
...
export function runCli(): void {
  ...
  watchAllPaths(args, getAutoIndexConfig() || DEFAULT_CONFIG);
}

 ...
 * src의 watch-all 모듈을 dynamic import로 로드합니다.
 */
...
async function main() {
  try {
    ...
    // Load compiled watch-all module from dist using dynamic import
    const { runCli } = await import('../dist/watch-all.js');

    // Execute CLI
    runCli();
  } catch (error) {
    console.error('❌ Error running watch-all:', error.message);
    console.error('');
